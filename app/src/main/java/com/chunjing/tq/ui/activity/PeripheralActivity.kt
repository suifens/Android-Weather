...
class PeripheralActivity : BaseVmActivity<ActivityPeripheralBinding, PeripheralViewModel>() {
    ...
    companion object {
        fun startActivity(context: Context, cities: ArrayList<CityEntity>) {
    ...
    private var mCityList: ArrayList<CityEntity>? = null
    private var mAdapter: PeripheralAdapter? = null

    @RequiresApi(Build.VERSION_CODES.TIRAMISU)
    override fun prepareData(intent: Intent?) {
        intent?.let {
            val array = it.getSerializableExtra("cities") as ArrayList<CityEntity>?
            if (array != null) {

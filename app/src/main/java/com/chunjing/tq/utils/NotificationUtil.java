...
//            PendingIntent contentIntent = PendingIntent.getActivity(context, 0, intent, PendingIntent.FLAG_MUTABLE);
    ...
    public static void updateNotification(Context context, int notifyId, String cityName, WeatherBean now) {
        NotificationCompat.Builder builder = notificationMap.get(notifyId);
        if (builder == null) {
            return;
        }
        ...
        if (now != null) {
            Observation observation = now.getObservation();
            views.setTextViewText(R.id.tvWeather, observation.getWxcPhrase());
            views.setTextViewText(R.id.tvTemp, observation.getMetric().getTemp() + "°C");
            views.setImageViewResource(R.id.ivWeather, WeatherUtils.getIcon(observation.getWxIcon()));

            views.setTextViewText(R.id.tvUpdateTime, "更新于：" + TimeUtils.millis2String(System.currentTimeMillis(), "HH:mm"));
        }

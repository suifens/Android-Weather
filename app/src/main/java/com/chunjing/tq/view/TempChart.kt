...
//        LogUtil.e("min: " + minTemp + " max:" + maxTemp + " prev:" + prev + " curr: " + current + " next: " + next);
        ...
        lowTemp = current.metric?.minTemp ?: 0
        highTemp = current.metric?.maxTemp ?: 0
        mPrev = prev
        mNext = next
        lowText = "$lowTemp°C"
        highText = "$highTemp°C"
    ...
    private fun getEnds(daily: Daily): Pair<Int, Int> {
        val topY = ((maxTemp - (daily.metric?.maxTemp ?: 0)) * density + topBottom + textHeight).toInt()
        val bottomY = ((maxTemp - (daily.metric?.minTemp ?: 0)) * density + topBottom + textHeight).toInt()

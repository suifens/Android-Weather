...
class WeatherWidgetSmall : AppWidgetProvider() {
    override fun onUpdate(
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O && Settings.canDrawOverlays(context)) {
            // todo app前台时启动前台服务
            context.startForegroundService(Intent(context, WidgetServiceSmall::class.java))
        } else {
            context.startService(Intent(context, WidgetServiceSmall::class.java))
        }
    ...
    override fun onDisabled(context: Context) {
        context.stopService(Intent(context, WidgetServiceSmall::class.java))
    }

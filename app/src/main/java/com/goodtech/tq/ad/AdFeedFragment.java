...
public class AdFeedFragment extends BaseFragment {
    ...
    protected void loadFeedAd(String codeId, int width, DataCallback<TTFeedAd> callback) {
        ...
        // 2、创建TTAdNative对象 */
        TTAdNative adNativeLoader = TTAdSdk.getAdManager().createAdNative(requireActivity());
    ...
    private void showFeedAd(FrameLayout feedContainer, TTFeedAd mTTFeedAd) {
        ...
        if (manager != null) {
            if (manager.isExpress()) { // --- 模板feed流广告
                ...
                View feedView = FeedAdUtils.getFeedAdFromFeedInfo(mTTFeedAd, requireActivity(), null, new TTNativeAd.AdInteractionListener() {
                    @Override

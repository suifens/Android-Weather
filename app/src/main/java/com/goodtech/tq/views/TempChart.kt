...
class TempChart @JvmOverloads constructor(
    ...
    init {
        ...
        mTextPaint.color = resources.getColor(R.color.color_d8d8d8)
        ...
        mLowPaint.color = Color.parseColor("#FFD34E")
        mHighPaint = Paint(Paint.ANTI_ALIAS_FLAG)
        mHighPaint.strokeWidth = lineWidth.toFloat()
        mHighPaint.strokeCap = Paint.Cap.SQUARE
        mHighPaint.color = Color.parseColor("#00C4FF")
        pntRadius = SizeUtils.dp2px(3f).toFloat()
    }
...
//        LogUtil.e("min: " + minTemp + " max:" + maxTemp + " prev:" + prev + " curr: " + current + " next: " + next);
        ...
        lowTemp = current.metric.minTemp
        highTemp = current.metric.maxTemp
        mPrev = prev
        mNext = next
        lowText = "$lowTemp°"
        highText = "$highTemp°"
    ...
    private fun getEnds(daily: Daily): Pair<Int, Int> {
        val topY = ((maxTemp - daily.metric.maxTemp) * density + topBottom + textHeight).toInt()
        val bottomY = ((maxTemp - daily.metric.minTemp) * density + topBottom + textHeight).toInt()

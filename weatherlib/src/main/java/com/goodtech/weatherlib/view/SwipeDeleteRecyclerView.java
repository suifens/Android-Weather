...
//                //LogUtils.e("onInterceptTouchEvent() -> ACTION_DOWN");
...
//                    mFlingView = (ViewGroup) getChildAt(mPosition - ((LinearLayoutManager) getLayoutManager()).findFirstVisibleItemPosition());
                    ...
                    if (view != null && mFlingView != view && view.getScrollX() != 0) {
                        view.scrollTo(0, 0);
                        //LogUtils.e("onInterceptTouchEvent() -> ACTION_DOWN 拦截事件-关闭已打开的menu");
                        return true;
                    }
...
//                //LogUtils.e("onInterceptTouchEvent() -> ACTION_MOVE");
                ...
                if (Math.abs(xVelocity) > SNAP_VELOCITY && Math.abs(xVelocity) > Math.abs(yVelocity)
                    ...
                    //LogUtils.e("onInterceptTouchEvent() -> ACTION_MOVE true");
                    mIsSlide = true;
...
//            Log.e("wsjLib", "getCurrX: " + mScroller.getCurrX());
     ...
     * @param width 子view宽度
     */
    public void showDeleteMenu(int position, int width) {
        closeMenu();
        mPosition = position;
        mFlingView = (ViewGroup) getChildAt(position);
        if (mFlingView != null && mFlingView.getScrollX() == 0) {
            mFlingView.scrollTo(width, 0);
        }
    }
